...
def combine_csvs():
    if not os.path.exists(data_folder):
        print(f"Data folder '{data_folder}' does not exist. Please run the update_data.py script first.")
        return

    csv_files = [f for f in os.listdir(data_folder) if f.endswith('.csv')]

    if not csv_files:
        print(f"No CSV files found in '{data_folder}'. Please run the update_data.py script first.")
        return
